...
void list_free(list_t *l){
  // Note: does not free blocks themselves, just the list structure
  free(l);
  return;
}

void node_free(node_t *node){
  // does not free block, just the node
  free(node);
  return;
}
...
void list_add_at_index(list_t *l, block_t *blk, int index){
  ...
  if(index <= 0 || l->head == NULL) {
    // add to front if index <=0 or list empty
    newNode->next = l->head;
    l->head = newNode;
    return;
  }
...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
    ...
    // If list is empty or newblk should be placed at head
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
    ...
    // If new block should be at the front
    int head_size = (l->head->blk->end - l->head->blk->start + 1);
    if (newblk_size < head_size) {
        newNode->next = l->head;
        l->head = newNode;
        return;
    }
...
void list_add_descending_by_blocksize(list_t *l, block_t *blk){
  ...
  int curblk_size = (current->blk->end - current->blk->start + 1);
...
/* Returns the index at which a block of Size or greater first appears. */
